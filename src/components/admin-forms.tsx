...
export function UserImportForm() {
  ...
  return <form onSubmit={submit} className="grid gap-4"><Field label="账号 Excel"><input name="usersFile" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required /></Field><p className="text-xs text-slate-500">从左到右四列依次为：用户名、初始密码、角色、小组。角色支持 SUPER_ADMIN / ADMIN / USER；普通用户必须填写已存在的小组。</p>{error && <p className="text-sm text-red-600">{error}</p>}{message && <p className="text-sm text-emerald-700">{message}</p>}<Button>{pending ? "导入中..." : "批量导入账号"}</Button></form>;
}
...
export function ModelRankingButton() {
  ...
  const [pending, setPending] = useState(false);
  async function run() {
    if (!confirm("确定对所有启用模型创建一轮排名评测？")) return;
    setPending(true);
    setError("");
    const res = await fetch("/api/admin/model-rankings", { method: "POST" });
    setPending(false);
    if (!res.ok) { const data = await res.json().catch(() => ({})); return setError(data.error ?? "创建排名失败"); }
    router.refresh();
  }
  return <div className="grid gap-2"><button type="button" onClick={run} disabled={pending} className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{pending ? "创建中..." : "开始模型排名"}</button>{error && <p className="text-sm text-red-600">{error}</p>}</div>;
}
