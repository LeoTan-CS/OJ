...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) { return handle(async () => { await requireAdmin(); const { id } = await params; const body = await parseJson(request, announcementSchema); return json({ announcement: await prisma.announcement.update({ where: { id }, data: { title: body.title, body: body.body, classId: body.classId ?? null } }) }); }); }
