...
function QuestionReportBlock({ identities, snapshot }: { identities?: ModelIdentityLookup; snapshot: LeaderboardSnapshot }) {
  return (
          ...
          <tbody>
            {snapshot.entries.map((entry) => (
              <tr key={`${snapshot.batch.id}-${entry.modelId}`}>
                <td><ModelIdentity {...resolveModelIdentity(entry, identities)} /></td>
                ...
                <table>
                  ...
                  <tbody>
                    {question.entries.map((entry) => {
                      ...
                      const identity = resolveModelIdentity(entry, identities);
                      return (
                        <tr key={`${question.questionId}-${entry.modelId}`}>
                          <td><ModelIdentity {...identity} /></td>
    ...
    <AppShell admin>
      ...
      <div className="grid gap-6">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card><div className="text-sm text-slate-500">排名题库</div><div className="mt-2 text-sm font-medium leading-6">{questionSummary}</div></Card>
        </div>
        <Card>
        ...
        <Card>
          <h2 className="text-xl font-bold">参与模型</h2>
          <table className="mt-3">
            <tbody>
              {models.map((model) => (
                <tr key={model.id}>
              ...
              {!models.length && <tr><td className="text-sm text-slate-500">暂无模型。</td></tr>}
            </tbody>
          </table>
        </Card>
        <Card>
          <h2 className="text-xl font-bold">排名批次</h2>
            ...
            {batches.map((batch, index) => {
              ...
              return (
                ...
                >
                  ...
                  {snapshot ? <QuestionReportBlock identities={identityByModelId} snapshot={snapshot} /> : rankings.length > 0 && (
                    <div className="mt-5">
                      <h3 className="font-bold">裁判排名</h3>
                      <table className="mt-3">
                        <tbody>
                          {rankings.map((ranking) => {
                            const identity = identityByModelId.get(ranking.modelId);
                            return (
                  ...
                  {!snapshot && <LegacyReportBlock reportText={batch.judgeReport} />}
                  {batch.judgeError && <div className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-700">{batch.judgeError}</div>}
                  {batch.judgeInputPath && <div className="mt-3 break-all text-xs text-slate-500">裁判输入 JSON：{batch.judgeInputPath}</div>}
                </CollapsiblePanel>
              );
            })}
            {!batches.length && <p className="text-sm text-slate-500">还没有排名批次。</p>}
          </div>
        </Card>
      </div>
    </AppShell>
