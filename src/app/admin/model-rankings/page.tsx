...
export default async function AdminModelRankingsPage() {
  ...
  return <AppShell admin><AdminNav /><div className="grid gap-6"><div className="grid gap-4 md:grid-cols-3"><Card><div className="text-sm text-slate-500">模型总数</div><div className="mt-2 text-3xl font-bold">{models.length}</div></Card><Card><div className="text-sm text-slate-500">启用模型</div><div className="mt-2 text-3xl font-bold">{enabledCount}</div></Card><Card><div className="text-sm text-slate-500">排名题库</div><div className="mt-2 text-sm font-medium leading-6">{questionSummary}</div></Card></div><Card><div className="flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-xl font-bold">模型排名</h1><p className="mt-1 text-sm text-slate-500">点击后会按题目顺序执行：先让所有启用小组模型完成当前题，再由裁判大模型对这一题生成质量排名与报告，然后进入下一题。</p></div><ModelRankingButton /></div></Card><Card><h2 className="text-xl font-bold">参与模型</h2><table className="mt-3"><tbody>{models.map((model) => <tr key={model.id}><td><div className="font-semibold">{model.name}</div><div className="text-xs text-slate-500">小组 {model.group?.name ?? model.name} · 上传人 {model.user.username} · {model.originalFilename}</div></td><td>{model.enabled ? <span className="text-sm font-medium text-emerald-700">启用</span> : <span className="text-sm font-medium text-slate-500">禁用</span>}</td><td>{formatDate(model.createdAt)}</td></tr>)}{!models.length && <tr><td className="text-sm text-slate-500">暂无模型。</td></tr>}</tbody></table></Card><Card><h2 className="text-xl font-bold">排名批次</h2><div className="mt-4 grid gap-6">{batches.map((batch, index) => { const snapshot = snapshotMap.get(batch.id); const rankings = parseJson<JudgeRanking[]>(batch.judgeRankingsJson) ?? []; return <CollapsiblePanel key={batch.id} defaultExpanded={index === 0} className="rounded-xl border p-4" header={<div><div className="font-semibold">批次 {batch.id}</div><div className="text-xs text-slate-500">创建人 {batch.createdBy.username} · {formatDate(batch.createdAt)}</div><div className="mt-1 text-xs text-slate-500">题库：{batch.question ?? questionSummary}</div></div>} actions={<><StatusBadge status={batch.status} /><span className="text-xs text-slate-500">裁判</span><StatusBadge status={batch.judgeStatus} /><DeleteButton endpoint={`/api/admin/model-tests/${batch.id}`} /></>}><table className="mt-3"><tbody>{batch.results.map((result) => <tr key={result.id}><td><div className="font-medium">{result.model.name}</div><div className="text-xs text-slate-500">小组 {result.model.group?.name ?? result.model.name} · 上传人 {result.model.user.username}</div></td><td><StatusBadge status={result.status} /></td><td>{result.durationMs ? `${result.durationMs}ms` : "-"}</td><td className="max-w-md"><div className="truncate text-xs text-slate-500">{result.outputPath ?? result.error ?? "-"}</div>{result.outputPreview && <details className="mt-1 text-xs"><summary className="cursor-pointer text-slate-600">回答预览</summary><pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded bg-slate-50 p-2">{result.outputPreview}</pre></details>}{result.error && <div className="mt-1 text-xs text-red-600">{result.error}</div>}</td></tr>)}</tbody></table>{snapshot ? <QuestionReportBlock snapshot={snapshot} /> : rankings.length > 0 && <div className="mt-5"><h3 className="font-bold">裁判排名</h3><table className="mt-3"><tbody>{rankings.map((ranking) => <tr key={`${batch.id}-${ranking.modelId}`}><td className="w-16 text-lg font-black">#{ranking.rank}</td><td><div className="font-semibold">{ranking.modelName}</div><div className="text-xs text-slate-500">{ranking.modelId}</div></td><td>{ranking.score == null ? "-" : `${ranking.score} 分`}</td><td className="text-sm text-slate-600">{ranking.reason}</td></tr>)}</tbody></table></div>}{!snapshot && <LegacyReportBlock reportText={batch.judgeReport} />}{batch.judgeError && <div className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-700">{batch.judgeError}</div>}{batch.judgeInputPath && <div className="mt-3 break-all text-xs text-slate-500">裁判输入 JSON：{batch.judgeInputPath}</div>}</CollapsiblePanel>; })}{!batches.length && <p className="text-sm text-slate-500">还没有排名批次。</p>}</div></Card></div></AppShell>;
}
